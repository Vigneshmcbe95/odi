...
-- Erstellt NUR FEHLENDE Tabellen in SVS41_DWH_FST, Quelle THM_DWH_FST.
--
-- Gleiche Regel wie bei SVS41WH_BA_TRS/THM_DWH_BA_TRS und
-- SVS41WH_FST/THM_DWH_FST (Thorsten, bestaetigt 2026-08-31): wenn
-- eine Tabelle sowohl im Basisschema als auch im 43er-Sandbox-Pendant
-- fehlt, ist THM_* der richtige Fallback.
--
-- WICHTIG -- persistentes Schema, kein Scratch. Deshalb hier bewusst
-- KEIN blindes Drop+Recreate -- eine bereits vorhandene, evtl. schon
-- befuellte Tabelle wird NICHT angefasst. Nur echte Luecken (Tabelle
-- existiert im Ziel gar nicht) werden neu angelegt.

declare
v_source_owner varchar2(30) := 'THM_DWH_FST';
v_target_owner varchar2(30) := 'SVS41_DWH_FST';
