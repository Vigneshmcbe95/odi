...
-- Erstellt Synonyme in SVS41WL_FST, die auf die Basistabellen in
-- PSD1_DWL_FST zeigen -- KEINE echten Tabellenkopien.
--
-- Hintergrund (Thorsten, 2026-09-03/04): fuer SVS41WL_FST werden keine
-- eigenen Ladetabellen mit Testdaten benoetigt (mangels Testdaten im
-- Filesystem, zumindest fuer CoSach) -- Synonyme auf PSD1 reichen.
-- Gleiches Muster wie bereits fuer SVS41WH_UEB_DIM verwendet.
--
-- VORAUSSETZUNG: alle vorher angelegten echten Tabellen in
-- SVS41WL_FST muessen bereits entfernt sein (siehe
-- svs41wl_fst_tabellen_droppen.sql) -- Tabelle und Synonym koennen
-- nicht denselben Namen im selben Schema haben.
--
...
DECLARE
  v_src_schema VARCHAR2(128) := 'PSD1_DWL_FST';
  v_tgt_schema VARCHAR2(128) := 'SVS41WL_FST';
