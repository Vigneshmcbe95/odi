-- Gleiches Muster wie bei W301/W302: W310_loaded_variables fehlen
-- vermutlich mehrere Variablen (STS_KA, GEM_KA, SGB_TRG_KA
-- aufgetreten). Gesamtvergleich statt Variable-fuer-Variable.
--
-- VOR jedem weiteren Fix: in Designer pruefen, ob
-- PCK_DWH_W310_000_SET_LOCAL_VARIABLES existiert -- falls ja, DAS
-- einmal laufen lassen statt hier einzeln nachzuziehen.

-- 1) Ist-Stand SSC41.
SELECT COUNT(*) AS anzahl_ssc41 FROM SSC41WH_FST.W310_loaded_variables;

-- 2) Referenz SCR_DWH_FST.
SELECT COUNT(*) AS anzahl_scr FROM SCR_DWH_FST.W310_loaded_variables;

-- 3) Welche Variablen fehlen komplett in SSC41?
SELECT scr.var_name
FROM SCR_DWH_FST.W310_loaded_variables scr
WHERE NOT EXISTS (
        SELECT 1 FROM SSC41WH_FST.W310_loaded_variables ssc
