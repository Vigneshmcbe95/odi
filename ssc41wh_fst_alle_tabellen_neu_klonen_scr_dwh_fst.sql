...
-- der Struktur aus SCR_DWH_FST -- nicht mehr aus SSC45WH_FST/SVS44.
--
-- Grund fuer den Wechsel der Quelle (Thorsten, Chat mit Joern,
-- 2026-09-18): Tabellen, die ueber SVN/SVS44 nach SVS41/B05 gekommen
-- sind, passen nicht immer zu den Mappings aus R01 (Beispiel:
-- W335_EVLB_LOAD, dessen Mapping nur in B03 existiert). Thorstens
-- Antwort: bei Abweichungen/veralteten SVN-Tabellen SCR_DWH_FST als
-- Referenz nehmen -- das ist das Schema, das wirklich zu R01 passt.
--
-- SSC* ist Scratch (temporaer) -- kein Datenverlust relevant, wird
-- beim naechsten Ladelauf sowieso neu befuellt.
--
-- ACHTUNG: das ist eine grosse, destruktive Aktion -- ALLE Tabellen
-- in SSC41WH_FST werden gedroppt und durch die SCR_DWH_FST-Struktur
-- ersetzt, auch die, die aktuell schon korrekt waren.
--
-- Vor dem Ausfuehren pruefen: existiert SCR_DWH_FST ueberhaupt als
-- Schema/Owner in dieser DB, und hat es alle benoetigten Tabellen
-- (z.B. W105_TMP_BAK_LOAD, W105_WRK_LOAD_COSACHNT_KUNDE)? Siehe
-- pruefen_w105_objekte_scr_dwh_fst.sql fuer den Existenz-Check.
...
begin
  ...
  dbms_output.put_line('Neu angelegt: '||v_erstellt||', Fehlgeschlagen: '||v_fehler);
