...
-- Erstellt Tabellen INKLUSIVE Primary Keys und Indizes von SVS43WH_FST nach
-- SVS41WH_FST -- vollstaendiger Klon, nichts wird bewusst weggelassen.
--
-- Wichtig zum Verstaendnis (Ursache des vorherigen ORA-00922):
...
-- einem CLOB. Mit SQLTERMINATOR=FALSE (vorherige Version) gingen die
-- trennenden ';' verloren, wodurch beide Anweisungen zu einer einzigen,
-- ungueltigen Anweisung verschmolzen sind.
...
--
-- Zusaetzlich: separate Indizes (die nicht Teil eines Primary/Unique Key
-- sind) werden in einem zweiten Durchlauf ebenfalls geklont.
...
DECLARE
  ...
  -- jede einzeln aus. Ein Fehler in einer Anweisung stoppt nicht die
  -- naechste (z.B. CREATE TABLE gelingt, ADD PRIMARY KEY schlaegt fehl).
  ...
  BEGIN
    ...
    -- letzter Rest ohne abschliessendes ';'
